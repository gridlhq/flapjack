 ...
 *
 * Integration tests that verify analytics data is correctly rolled up,
 * processed, and displayed. Seeds real analytics data before running tests.
 *
...
const INDEX = 'test-analytics-integration';

// Expected values based on DEFAULT_ANALYTICS_CONFIG
const EXPECTED = {
  totalSearches: DEFAULT_ANALYTICS_CONFIG.searchCount,
  uniqueUsers: 50, // Generated by analytics-seed
...
// ─── Analytics Data Verification (with real data) ──────────────────

test.describe('Analytics Data Verification', () => {
  // Seed analytics data once for all tests in this file
...
test.describe('API-Level Data Verification', () => {
  ...
  test('search count returns positive total with daily breakdown', async ({ request }) => {
    ...
    expect(data.count).toBeGreaterThanOrEqual(EXPECTED.totalSearches * 0.9); // Allow 10% variance
  ...
  test('unique users count is positive and less than total searches', async ({ request }) => {
    ...
    expect(users.count).toBeGreaterThanOrEqual(EXPECTED.uniqueUsers * 0.9); // Allow variance
  ...
  test('no-result rate is between 0 and 1 with daily breakdown', async ({ request }) => {
    ...
    expect(data.rate).toBeCloseTo(EXPECTED.noResultRate, 1); // Within 0.1
    expect(data.dates).toBeDefined();
    expect(data.dates.length).toBeGreaterThan(0);
  ...
  test('device breakdown sums to total searches', async ({ request }) => {
    ...
    // Desktop should be the largest (60% in seed config)
    const desktop = platforms.find((p: any) => p.platform === 'desktop');
    expect(desktop).toBeDefined();
    expect(desktop.count).toBeGreaterThan(platformSum * (EXPECTED.desktopPct - 0.1)); // Allow 10% variance
  });
  ...
  test('geo breakdown sums to total searches', async ({ request }) => {
    ...
    expect(geo.countries.length).toBeGreaterThanOrEqual(3); // At least US, GB, DE, CA, FR
    ...
    // US should be the top country (45% in seed config)
    expect(geo.countries[0].country).toBe('US');
    expect(geo.countries[0].count).toBeGreaterThan(geo.total * (EXPECTED.usPct - 0.1)); // Allow 10% variance
  ...
  test.skip('US regions sum to US country count', async ({ request }) => {
    // TODO: Implement regional data seeding
    const now = new Date();
    const startDate = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
    const endDate = now.toISOString().split('T')[0];
    ...
    expect(regions.regions.length).toBeGreaterThanOrEqual(1);
  ...
  test('country filter produces subset of total searches', async ({ request }) => {
    const [allRes, usRes, gbRes] = await Promise.all([
      request.get(`${API}/2/searches`, {
        params: { index: INDEX, ...getDateRange(), limit: '5' },
        headers: H,
      }),
      request.get(`${API}/2/searches`, {
        params: { index: INDEX, ...getDateRange(), limit: '5', country: 'US' },
        headers: H,
      }),
      request.get(`${API}/2/searches`, {
        params: { index: INDEX, ...getDateRange(), limit: '5', country: 'GB' },
        headers: H,
      }),
    ]);
  ...
  test('device filter produces subset of total searches', async ({ request }) => {
    const [allRes, desktopRes] = await Promise.all([
      ...
      request.get(`${API}/2/searches`, {
        params: { index: INDEX, ...getDateRange(), limit: '5', tags: 'platform:desktop' },
        headers: H,
      }),
    ]);
...
test.describe('Analytics UI Data Verification', () => {
  ...
  test.describe('Overview Tab', () => {
    test('KPI cards show non-zero values from API data', async ({ page }) => {
      ...
      expect(searchNum).toBeGreaterThanOrEqual(EXPECTED.totalSearches * 0.9); // Allow 10% variance
