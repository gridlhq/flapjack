...
test.describe('Search & Browse', () => {
  ...
  test('searching for "laptop" returns laptop products', async ({ page }) => {
    ...
    // All three laptops should appear in results
    await expect(page.getByText('MacBook Pro').first()).toBeVisible({ timeout: 10000 });
    await expect(page.getByText('ThinkPad X1 Carbon').first()).toBeVisible();
    await expect(page.getByText('Dell XPS 15').first()).toBeVisible();
  });
  ...
  test('filtering by Audio category shows only audio products', async ({ page }) => {
    ...
    // Click on "Audio" in the category facet
    await facetsPanel.locator('button', { hasText: 'Audio' }).click();

    // Wait for results to update with the filter applied
    const resultsPanel = page.locator('[data-testid="results-panel"]');
    await expect(resultsPanel).toBeVisible({ timeout: 10000 });

    // Should see audio products
    await expect(page.getByText('Sony WH-1000XM5').first()).toBeVisible({ timeout: 10000 });
    await expect(page.getByText('AirPods Pro 2').first()).toBeVisible();

    // Should NOT see non-audio products
    await expect(page.getByText('MacBook Pro').first()).not.toBeVisible();
    await expect(page.getByText('Dell XPS 15').first()).not.toBeVisible();
  });
  ...
  test('filtering by Apple brand shows only Apple products', async ({ page }) => {
    ...
    // Click on "Apple" in the brand facet
    await facetsPanel.locator('button', { hasText: 'Apple' }).click();

    // Wait for results to update
    const resultsPanel = page.locator('[data-testid="results-panel"]');
    await expect(resultsPanel).toBeVisible({ timeout: 10000 });

    // Should see Apple products (MacBook Pro, iPad Pro, AirPods Pro)
    await expect(page.getByText('MacBook Pro').first()).toBeVisible({ timeout: 10000 });
    await expect(page.getByText('iPad Pro').first()).toBeVisible();
    await expect(page.getByText('AirPods Pro').first()).toBeVisible();

    // Should NOT see non-Apple products
    await expect(page.getByText('ThinkPad X1 Carbon').first()).not.toBeVisible();
    await expect(page.getByText('Sony WH-1000XM5').first()).not.toBeVisible();
    await expect(page.getByText('Galaxy Tab S9').first()).not.toBeVisible();
  });
  ...
  test('combining category and brand facets narrows results', async ({ page }) => {
    ...
    await facetsPanel.locator('button', { hasText: 'Laptops' }).click();
  ...
  test('clicking Search button triggers search', async ({ page }) => {
    ...
    const resultsPanel = page.locator('[data-testid="results-panel"]');
    await expect(resultsPanel).toBeVisible({ timeout: 10000 });
    await expect(page.getByText('Galaxy Tab S9').first()).toBeVisible({ timeout: 10000 });
  });
  ...
  // ---------------------------------------------------------------------------
  test('facets panel shows all category values', async ({ page }) => {
    const facetsPanel = page.locator('[data-testid="facets-panel"]');
    await expect(facetsPanel).toBeVisible({ timeout: 10000 });

    // All 6 categories should be visible as facet buttons
    await expect(facetsPanel.getByText('Laptops').first()).toBeVisible();
    await expect(facetsPanel.getByText('Tablets').first()).toBeVisible();
    await expect(facetsPanel.getByText('Audio').first()).toBeVisible();
    await expect(facetsPanel.getByText('Storage').first()).toBeVisible();
    await expect(facetsPanel.getByText('Monitors').first()).toBeVisible();
    await expect(facetsPanel.getByText('Accessories').first()).toBeVisible();
  });
  ...
  test('facets panel shows brand facet values', async ({ page }) => {
    ...
    // Major brands should be visible
    await expect(facetsPanel.getByText('Apple').first()).toBeVisible();
    await expect(facetsPanel.getByText('Samsung').first()).toBeVisible();
    await expect(facetsPanel.getByText('Sony').first()).toBeVisible();
  });
  ...
  test('facet values show document counts', async ({ page }) => {
    ...
    // Laptops should show count "3" (MacBook, ThinkPad, Dell XPS)
    // Apple should show count "3" (MacBook, iPad, AirPods)
    // Look for numeric badges within facet buttons
    const laptopsBtn = facetsPanel.locator('button', { hasText: 'Laptops' });
    await expect(laptopsBtn).toBeVisible();
    await expect(laptopsBtn.getByText('3')).toBeVisible();
  });
});
