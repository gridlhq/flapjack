/**
 * Code generated by OpenAPI Generator (https://openapi-generator.tech), manual changes will be
 * lost - read more on https://github.com/flapjackhq/sdk-automation. DO NOT EDIT.
 */
package com.flapjackhq.client.model.search

import com.flapjackhq.client.exception.AlgoliaClientException
import com.flapjackhq.client.extensions.internal.*
import kotlin.jvm.JvmInline
import kotlinx.serialization.*
import kotlinx.serialization.builtins.*
import kotlinx.serialization.descriptors.*
import kotlinx.serialization.encoding.*
import kotlinx.serialization.json.*

/**
 * BrowseParams
 *
 * Implementations:
 * - [BrowseParamsObject]
 * - [SearchParamsString]
 */
@Serializable(BrowseParamsSerializer::class)
public sealed interface BrowseParams {
  @Serializable
  @JvmInline
  public value class SearchParamsStringValue(public val value: SearchParamsString) : BrowseParams

  @Serializable
  @JvmInline
  public value class BrowseParamsObjectValue(public val value: BrowseParamsObject) : BrowseParams

  public companion object {

    public fun of(value: SearchParamsString): BrowseParams = SearchParamsStringValue(value)

    public fun of(value: BrowseParamsObject): BrowseParams = BrowseParamsObjectValue(value)
  }
}

internal class BrowseParamsSerializer :
  JsonContentPolymorphicSerializer<BrowseParams>(BrowseParams::class) {
  override fun selectDeserializer(element: JsonElement): DeserializationStrategy<BrowseParams> {
    return when {
      element is JsonObject && element.containsKey("params") -> SearchParamsString.serializer()
      element is JsonObject -> BrowseParamsObject.serializer()
      else -> throw AlgoliaClientException("Failed to deserialize json element: $element")
    }
  }
}
