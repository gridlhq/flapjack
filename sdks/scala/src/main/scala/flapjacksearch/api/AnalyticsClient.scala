...
import flapjacksearch.analytics.Direction._
import flapjacksearch.analytics.ErrorBase
import flapjacksearch.analytics.GetAddToCartRateResponse
import flapjacksearch.analytics.GetAverageClickPositionResponse
import flapjacksearch.analytics.GetClickPositionsResponse
import flapjacksearch.analytics.GetClickThroughRateResponse
import flapjacksearch.analytics.GetConversionRateResponse
import flapjacksearch.analytics.GetNoClickRateResponse
import flapjacksearch.analytics.GetNoResultsRateResponse
import flapjacksearch.analytics.GetPurchaseRateResponse
import flapjacksearch.analytics.GetRevenue
import flapjacksearch.analytics.GetSearchesCountResponse
import flapjacksearch.analytics.GetSearchesNoClicksResponse
import flapjacksearch.analytics.GetSearchesNoResultsResponse
import flapjacksearch.analytics.GetStatusResponse
import flapjacksearch.analytics.GetTopCountriesResponse
import flapjacksearch.analytics.GetTopFilterAttributesResponse
import flapjacksearch.analytics.GetTopFilterForAttributeResponse
import flapjacksearch.analytics.GetTopFiltersNoResultsResponse
import flapjacksearch.analytics.GetTopHitsResponse
import flapjacksearch.analytics.GetTopSearchesResponse
import flapjacksearch.analytics.GetUsersCountResponse
import flapjacksearch.analytics.OrderBy._
import flapjacksearch.analytics._
import flapjacksearch.ApiClient
import flapjacksearch.api.AnalyticsClient.hosts
import flapjacksearch.api.AnalyticsClient.readTimeout
import flapjacksearch.api.AnalyticsClient.writeTimeout
import flapjacksearch.api.AnalyticsClient.connectTimeout
...
object AnalyticsClient {
  ...
  private def hosts(region: Option[String] = None): Seq[Host] = {
    ...
    val url =
      if (region.isEmpty) "analytics.flapjack.io" else "analytics.{region}.flapjack.io".replace("{region}", region.get)
...
class AnalyticsClient(
  ...
  /** Retrieves the add-to-cart rate for all your searches with at least one add-to-cart event, including a daily
    ...
    * add-to-cart rate when `clickAnalytics` is enabled:** - **Null** means there were no queries: since Flapjack didn't
    * receive any events, the add-to-cart rate is null. - **0** mean there _were_ queries but no [add-to-cart
    * events](https://www.flapjack.io/doc/guides/sending-events/getting-started) were received.
    *
  ...
  /** Retrieves the average click position of your search results, including a daily breakdown. The average click
    ...
    * days including the current day. An average of `null` when `clickAnalytics` is enabled means Flapjack didn't receive
    * any [click events](https://www.flapjack.io/doc/guides/sending-events/getting-started) for the queries. The average
    * is `null` until Flapjack receives at least one click event.
    *
    * Required API Key ACLs:
    *   - analytics
    *
  ...
  /** Retrieves the positions in the search results and their associated number of clicks. This lets you check how many
    ...
    * means Flapjack didn't receive any [click events](https://www.flapjack.io/doc/guides/sending-events/getting-started)
    * for the queries.
    *
    * Required API Key ACLs:
    *   - analytics
  ...
  /** Retrieves the click-through rate (CTR) for all your searches with at least one click event, including a daily
    ...
    * since Flapjack didn't receive any events, CTR is null. - **0** mean there _were_ queries but no [click
    * events](https://www.flapjack.io/doc/guides/sending-events/getting-started) were received.
    *
    * Required API Key ACLs:
    *   - analytics
  ...
  /** Retrieves the conversion rate (CR) for all your searches with at least one conversion event, including a daily
    ...
    * since Flapjack didn't receive any events, CR is null. - **0** mean there _were_ queries but no [conversion
    * events](https://www.flapjack.io/doc/guides/sending-events/getting-started) were received.
    *
    * Required API Key ACLs:
    *   - analytics
  ...
  /** Retrieves the purchase rate for all your searches with at least one purchase event, including a daily breakdown.
    ...
    * there were no queries. Flapjack didn't receive any events, so the purchase rate is null. - **0** means there were
    * queries but no [purchase conversion events](https://www.flapjack.io/doc/guides/sending-events/getting-started)
    * were received.
    *
    * Required API Key ACLs:
    *   - analytics
  ...
  /** Retrieves the 1,000 most frequently used filters for a search that didn't return any results. To get the most
    ...
    * results](https://www.flapjack.io/doc/rest-api/analytics/get-searches-no-results) operation.
    *
    * Required API Key ACLs:
    *   - analytics
    *
  ...
  /** Retrieves the object IDs of the 1,000 most frequent search results. If you set the `clickAnalytics` query
    ...
    * **There's a difference between 0% rates and null rates:** - **Null** means there were no queries. Flapjack didn't
  ...
  /** Returns the most popular searches. For each search, it also includes the average number of hits. If you set the
    ...
    * queries. Flapjack didn't receive any events, so rates are null. - **0% rates** mean there were queries but no
    * [click or conversion events](https://www.flapjack.io/doc/guides/sending-events/getting-started) were received.
    *
    * Required API Key ACLs:
    *   - analytics
    *
