...
import flapjacksearch.exception.FlapjackClientException
import flapjacksearch.internal.interceptor.{AuthInterceptor, RetryStrategy, UserAgentInterceptor}
import flapjacksearch.internal.{FlapjackAgent, HttpRequester, StatefulHost}
...
abstract class ApiClient(
  ...
  if (appId == null || appId.isEmpty) {
    throw FlapjackClientException("`appId` is missing.")
  }
  if (apiKey == null || apiKey.isEmpty) {
    throw FlapjackClientException("`apiKey` is missing.")
  }
  ...
  private def defaultRequester(
    ...
    val flapjackAgent = FlapjackAgent(BuildInfo.version)
    ...
    val builder = HttpRequester
      ...
      .withInterceptor(new UserAgentInterceptor(flapjackAgent))
