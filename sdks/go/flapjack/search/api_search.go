...
import (
	...
	"github.com/flapjackhq/flapjack-search-go/v4/flapjack/ingestion"
...
ListIndices calls the API and returns the raw response from it.
	...
	Lists all indices in the current Flapjack application.
...
OperationIndex calls the API and returns the raw response from it.
	...
	Copies or moves (renames) an index within the same Flapjack application.
...
Flapjack stores up to 1,000 API keys per application.
...
If a rule with the specified object ID doesn't exist, Flapjack creates a new one.
...
SaveSynonym calls the API and returns the raw response from it.
	...
	If a synonym with the specified object ID doesn't exist, Flapjack adds a new one.
...
SaveSynonyms calls the API and returns the raw response from it.
	...
	If a synonym with the `objectID` doesn't exist, Flapjack adds a new one.
...
/*
SearchForHits calls the `search` method but with certainty that we will only request Flapjack records (hits) and not facets.
...
/*
SearchForFacets calls the `search` method but with certainty that we will only request Flapjack facets and not records (hits).
