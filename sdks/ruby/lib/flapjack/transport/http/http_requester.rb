module Flapjack
  module Http
    class HttpRequester
      ...
      # @param logger [Object] logger used to log requests. Defaults to Flapjack::LoggerHelper
      #
      ...
      def send_request(host, method, path, body, query_params, headers, timeout, connect_timeout)
        ...
        @logger.info("Sending #{method.to_s.upcase} request to #{path} with body #{body}") if ENV["FLAPJACK_DEBUG"]

        response = connection.run_request(method, path, body, headers)

        if response.success?
          if ENV["FLAPJACK_DEBUG"]
        ...
        @logger.info("Request timed out. Error: #{e.message}") if ENV["FLAPJACK_DEBUG"]
        Http::Response.new(error: e.message, has_timed_out: true)
      rescue ::StandardError => e
        @logger.info("Request failed. Error: #{e.message}") if ENV["FLAPJACK_DEBUG"]
