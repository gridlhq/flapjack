...
test.describe('Analytics', () => {
  ...
  test('date range toggle switches between 7d, 30d, 90d and triggers API calls', async ({ page }) => {
    ...
    // Click 30d — wait for API response instead of sleeping
    const response30d = page.waitForResponse(resp => resp.url().includes('/2/'), { timeout: 10_000 });
    await btn30d.click();
    await response30d;
    await expect(page.getByTestId('kpi-cards')).toBeVisible({ timeout: 10_000 });

    // Click 90d
    const response90d = page.waitForResponse(resp => resp.url().includes('/2/'), { timeout: 10_000 });
    await btn90d.click();
    await response90d;
    await expect(page.getByTestId('kpi-cards')).toBeVisible({ timeout: 10_000 });

    // Click back to 7d
    const response7d = page.waitForResponse(resp => resp.url().includes('/2/'), { timeout: 10_000 });
    await btn7d.click();
    await response7d;
    await expect(page.getByTestId('kpi-cards')).toBeVisible({ timeout: 10_000 });
  });
  ...
  test('Filters tab shows Top Filter Attributes table with data', async ({ page }) => {
    ...
    await expect(filtersTable.getByText('Attribute')).toBeVisible({ timeout: 10_000 });
    await expect(filtersTable.getByText('Count')).toBeVisible();

    // With seeded data, at least one filter row should appear
  ...
  test('Filters tab: clicking a filter row expands to show filter values', async ({ page }) => {
    ...
    // After expansion, nested value rows should appear (bg-accent/10 rows with indented text)
    // The expanded section shows filter values or a "Loading values..." / "No values found" message
    await expect(async () => {
