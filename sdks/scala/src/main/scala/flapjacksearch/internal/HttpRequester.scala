...
import flapjacksearch.exception.{FlapjackApiException, FlapjackClientException}
import flapjacksearch.internal.interceptor.{GzipRequestInterceptor, HeaderInterceptor, LogInterceptor}
import flapjacksearch.internal.util.escape
import flapjacksearch.internal.util.UseReadTransporter
import okhttp3._
...
/** HttpRequester is responsible for making HTTP requests using the OkHttp client. It provides a mechanism for request
  ...
  */
private[flapjacksearch] class HttpRequester private (
  ...
  private def createHttpUrl(
    ...
    val urlBuilder = new HttpUrl.Builder()
      .scheme("https")
      .host("flapjack.io") // will be overridden by the retry strategy
  ...
  override def execute[T: Manifest](
    ...
    try {
      ...
      if (!response.isSuccessful)
        throw FlapjackApiException(message = response.message, httpErrorCode = response.code)
      // Deserialize and return the response.
      jsonSerializer.deserialize[T](response.body.byteStream)
    } catch {
      case exception: IOException => throw FlapjackClientException(cause = exception)
      case exception: FlapjackApiException =>
        var message = ""
        try {
