...
final class ClusterHosts
{
    ...
    {
        $read = $write = [
            $applicationId.'-1.flapjacknet.io' => 0,
            $applicationId.'-2.flapjacknet.io' => 0,
            $applicationId.'-3.flapjacknet.io' => 0,
        ];

        $read[$applicationId.'-dsn.flapjack.io'] = 10;
        $write[$applicationId.'.flapjack.io'] = 10;
    ...
    {
        if (!Flapjack::isCacheEnabled()) {
            return false;
        }

        if (!Flapjack::getCache()->has($cacheKey)) {
            return false;
        }

        return @unserialize(Flapjack::getCache()->get($cacheKey));
    }
    ...
    {
        if (null !== $this->cacheKey && Flapjack::isCacheEnabled()) {
            Flapjack::getCache()->set($this->cacheKey, serialize($this));
        }
    }
}
