...
import flapjacksearch.config.CallType
import flapjacksearch.exception.{
  FlapjackApiException,
  FlapjackClientException,
  FlapjackRequestException,
  FlapjackRetryException
}
import flapjacksearch.internal.StatefulHost
import flapjacksearch.internal.interceptor.RetryStrategy.expirationThreshold
...
/** Interceptor that retries requests on failure.
  ...
  *   list of hosts
  */
private[flapjacksearch] class RetryStrategy(hosts: List[StatefulHost]) extends Interceptor {
  ...
  override def intercept(chain: Interceptor.Chain): Response = {
    ...
    throw FlapjackRetryException(errors.toList)
  }
  ...
  private def handleResponse(
    ...
    try {
      ...
      if (isRetryable(response)) {
        throw FlapjackRequestException(
          message = message,
          httpErrorCode = response.code()
        )
      } else {
        throw FlapjackApiException(
          message = message,
          httpErrorCode = response.code()
        )
      }
  ...
  private def handleException(
    ...
    exception match {
      ...
      case _: FlapjackRequestException | _: IOException =>
        currentHost.hasFailed()
      case e: FlapjackApiException => throw e
      case _                      => throw FlapjackClientException(cause = exception)
    }
  }
}
