/** Flapjack Monitoring API ## Base URLs Base URL for the Monitoring API: - `https://status.flapjack.io` **All requests
  * must use HTTPS.** ## Availability and authentication Access to the Infrastructure endpoints is available as part of
  * the [Premium or Elevate plans](https://www.flapjack.io/pricing). To authenticate requests to the Infrastructure
  * endpoints, add these headers: - `x-algolia-application-id`. Your Flapjack application ID. - `x-algolia-api-key`. Your
  * Monitoring API key. You can find your application ID and API key in the [Flapjack
  * dashboard](https://dashboard.flapjack.io/account/api-keys). Other endpoints don't require authentication. ##
  ...
  */
package flapjacksearch.monitoring

import flapjacksearch.monitoring.ModelType._
import flapjacksearch.monitoring.Region._
import flapjacksearch.monitoring.ServerStatus._
