    ...
    class RecommendUpdatedAtResponse
      ...
      def initialize(attributes = {})
        if (!attributes.is_a?(Hash))
          raise(
            ArgumentError,
            "The input argument (attributes) must be a hash in `Flapjack::RecommendUpdatedAtResponse` initialize method"
          )
        end
        ...
        attributes = attributes.each_with_object({}) { |(k, v), h|
          if (!self.class.attribute_map.key?(k.to_sym))
            raise(
              ArgumentError,
              "`#{k}` is not a valid attribute in `Flapjack::RecommendUpdatedAtResponse`. Please check the name to make sure it's valid. List of attributes: " +
