...
//
//
//  Created by Flapjack on 26/02/2024.
//
...
public extension SearchClient {
    ...
    func waitForApiKey(
        ...
        if operation == .update {
            guard let apiKey else {
                throw FlapjackError.runtimeError("Missing API key optimistic value")
            }
        ...
        return try await createIterable(
            execute: { _ in
                ...
                } catch let FlapjackError.httpError(error) {
    ...
    func indexExists(indexName: String) async throws -> Bool {
        ...
        } catch let FlapjackError.httpError(error) where error.statusCode == 404 {
            return false
        }
