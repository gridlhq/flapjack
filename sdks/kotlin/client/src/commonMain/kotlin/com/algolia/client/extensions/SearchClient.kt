package com.flapjackhq.client.extensions
...
import com.flapjackhq.client.extensions.internal.DisjunctiveFaceting
import com.flapjackhq.client.extensions.internal.buildRestrictionString
import com.flapjackhq.client.extensions.internal.encodeKeySHA256
import com.flapjackhq.client.extensions.internal.retryUntil
...
@Deprecated(
  ...
  ReplaceWith(
    "waitForTask(indexName, taskID, maxRetries, timeout, initialDelay, maxDelay, requestOptions)"
  ),
)
...
public suspend fun SearchClient.searchForHits(
  ...
  return search(searchMethodParams = request, requestOptions = requestOptions).results.map {
...
public suspend fun SearchClient.searchForFacets(
  ...
  val request = SearchMethodParams(requests = requests, strategy = strategy)
  return search(searchMethodParams = request, requestOptions = requestOptions).results.map {
    it as SearchForFacetValuesResponse
  }
}

/**
 * Helper: Chunks the given `objects` list in subset of 1000 elements max to make it fit in `batch`
 * requests.
 *
 * @param indexName The index in which to perform the request.
...
/**
 * Helper: Deletes every record for the given objectIDs. The `chunkedBatch` helper is used under the
 * hood, which creates a `batch` requests with at most 1000 objectIDs in it.
 *
 * @param indexName The index in which to perform the request.
 ...
 * field. The `chunkedBatch` helper is used under the hood, which creates a `batch` requests with at
 * most 1000 objects in it.
 *
 * @param indexName The index in which to perform the request.
 ...
 * untouched. Replace all objects in an index without any downtime. Internally, this method copies
 * the existing index settings, synonyms and query rules and indexes all passed objects. Finally,
 ...
 * @throws IllegalArgumentException if [apiKey] doesn't have a
 *   [SecuredApiKeyRestrictions.validUntil].
 */
public fun securedApiKeyRemainingValidity(apiKey: String): Duration {
  ...
  val match =
    requireNotNull(pattern.find(decoded)) {
      "The Secured API Key doesn't have a validUntil parameter."
    }
 ...
 * @param refinements Refinements to apply to the search in form of dictionary with facet attribute
 *   as a key and a list of facet values for the designated attribute. Any facet in this list not
 *   present in the `disjunctiveFacets` set will be filtered conjunctively (with AND operator).
 ...
 * @param validate The function to validate the response. Default is to check if the cursor is not
 *   null.
