 ...
 *
 * @param automaticFacetFilters Filter recommendations that match or don't match the same
 ...
 * @param optionalFilters Filters to promote or demote records in the search results. Optional
 *   filters work like facet filters, but they don't exclude records from the search results.
 *   Records that match the optional filter rank before records that don't match. Matches with
 *   higher weights (`<score=N>`) rank before matches with lower weights. If you're using a negative
 *   filter `facet:-value`, matching records rank after records that don't match.
 */
@Serializable
public data class ParamsConsequence(
  ...
  val automaticFacetFilters: List<AutoFacetFilter>? = null,
   ...
   */
  @SerialName(value = "optionalFilters") val optionalFilters: List<String>? = null,
) {}
