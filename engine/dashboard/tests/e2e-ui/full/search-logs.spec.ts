...
test.describe('Search Logs', () => {
  ...
  test('expanded log entry shows request body and response sections', async ({ page }) => {
    ...
    const postEntry = page.getByText('POST').first();
    if (await postEntry.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      // Should show request body and/or response section headings
      const requestBody = page.getByText('Request Body');
      const response = page.getByText('Response').first();
      await expect(requestBody.or(response)).toBeVisible({ timeout: 5000 });
    }
  });
