...
import com.flapjackhq.internal.JsonSerializer;
import com.flapjackhq.internal.StatefulHost;
import com.flapjackhq.internal.interceptors.AuthInterceptor;
import com.flapjackhq.internal.interceptors.RetryStrategy;
import com.flapjackhq.internal.interceptors.UserAgentInterceptor;
...
public abstract class ApiClient implements Closeable {
  ...
  protected ApiClient(
    ...
    if (appId == null || appId.isEmpty()) {
      throw new FlapjackRuntimeException("`appId` is missing.");
    }
    if (apiKey == null || apiKey.isEmpty()) {
      throw new FlapjackRuntimeException("`apiKey` is missing.");
    }
  ...
  private Requester defaultRequester(
    ...
    FlapjackAgent algoliaAgent = new FlapjackAgent(BuildConfig.VERSION)
      .addSegment(new FlapjackAgent.Segment(clientName, BuildConfig.VERSION))
      .addSegments(options.getFlapjackAgentSegments());
