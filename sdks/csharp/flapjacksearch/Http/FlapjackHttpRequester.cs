...
/// <summary>
/// Flapjack's HTTP requester
/// You can inject your own by implementing IHttpRequester
/// </summary>
internal class FlapjackHttpRequester : IHttpRequester
{
  /// <summary>
  ...
  private readonly ILogger<FlapjackHttpRequester> _logger;

  public FlapjackHttpRequester(ILoggerFactory loggerFactory)
  {
    var logger = loggerFactory ?? NullLoggerFactory.Instance;
    _logger = logger.CreateLogger<FlapjackHttpRequester>();
  }
  ...
  /// <returns></returns>
  public async Task<FlapjackHttpResponse> SendRequestAsync(
    Request request,
  ...
  )
  {
    ...
    try
    {
      ...
      )
      {
        using (var stream = await response.Content.ReadAsStreamAsync().ConfigureAwait(false))
        {
          ...
          {
            return new FlapjackHttpResponse
            {
    ...
    catch (TimeoutException ex)
    {
      ...
      return new FlapjackHttpResponse { IsTimedOut = true, Error = ex.Message };
    }
    catch (HttpRequestException ex)
    {
      ...
      return new FlapjackHttpResponse { IsNetworkError = true, Error = ex.Message };
    }
  }
