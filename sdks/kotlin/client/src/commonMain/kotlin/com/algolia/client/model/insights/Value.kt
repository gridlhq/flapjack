/**
 * Code generated by OpenAPI Generator (https://openapi-generator.tech), manual changes will be
 * lost - read more on https://github.com/flapjackhq/sdk-automation. DO NOT EDIT.
 */
package com.flapjackhq.client.model.insights

import com.flapjackhq.client.exception.AlgoliaClientException
import com.flapjackhq.client.extensions.internal.*
import kotlin.jvm.JvmInline
import kotlinx.serialization.*
import kotlinx.serialization.builtins.*
import kotlinx.serialization.descriptors.*
import kotlinx.serialization.encoding.*
import kotlinx.serialization.json.*

/**
 * Total monetary value of this event in units of `currency`. This should be equal to the sum of
 * `price` times `quantity`.
 *
 * Implementations:
 * - [Double] - *[Value.of]*
 * - [String] - *[Value.of]*
 */
@Serializable(ValueSerializer::class)
public sealed interface Value {
  @Serializable @JvmInline public value class DoubleValue(public val value: Double) : Value

  @Serializable @JvmInline public value class StringValue(public val value: String) : Value

  public companion object {

    public fun of(value: Double): Value = DoubleValue(value)

    public fun of(value: String): Value = StringValue(value)
  }
}

internal class ValueSerializer : JsonContentPolymorphicSerializer<Value>(Value::class) {
  override fun selectDeserializer(element: JsonElement): DeserializationStrategy<Value> {
    return when {
      element.isDouble -> Value.DoubleValue.serializer()
      element.isString -> Value.StringValue.serializer()
      else -> throw AlgoliaClientException("Failed to deserialize json element: $element")
    }
  }
}
