...
import flapjacksearch.config.RequestOptions
import flapjacksearch.exception.FlapjackApiException
import flapjacksearch.extension.internal.Iterable.createIterable
import flapjacksearch.extension.internal.RetryUntil.{DEFAULT_DELAY, retryUntil}
import flapjacksearch.internal.util.{escape, paramToString}
...
package object extension {
  ...
  implicit class SearchClientExtensions(val client: SearchClient) {
    ...
    /** Wait for a taskID to complete before executing the next line of code, to synchronize index updates. All write
      * operations in Flapjack are asynchronous by design. It means that when you add or update an object to your index,
      ...
      retryUntil(
        retry = () =>
          client
            ...
            .map(Some(_))
            .recover {
              case e: FlapjackApiException if e.httpErrorCode == 404 => None // The key does not exist, done!
    ...
    def indexExists(indexName: String)(implicit ec: ExecutionContext): Future[Boolean] = {
      client.getSettings(indexName).map(_ => true).recover {
        case apiError: FlapjackApiException if apiError.httpErrorCode == 404 => false
        case e                                                              => throw e
      }
    }
