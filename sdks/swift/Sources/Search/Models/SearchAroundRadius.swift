...
public enum SearchAroundRadius: Codable, JSONEncodable, AbstractEncodable {
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        case let .searchAroundRadiusAll(value):
            try container.encode(value)
        }
    }
    ...
    public init(from decoder: Decoder) throws {
        ...
        if let value = try? container.decode(Int.self) {
            self = .int(value)
        } else if let value = try? container.decode(SearchAroundRadiusAll.self) {
            self = .searchAroundRadiusAll(value)
        } else {
            throw DecodingError.typeMismatch(
                Self.Type.self,
                .init(
                    codingPath: decoder.codingPath,
                    debugDescription: "Unable to decode instance of SearchAroundRadius"
                )
            )
        }
    }
    ...
    public func GetActualInstance() -> Encodable {
        ...
        case let .int(value):
            value as Int
        case let .searchAroundRadiusAll(value):
            value as SearchAroundRadiusAll
        }
    }
}
