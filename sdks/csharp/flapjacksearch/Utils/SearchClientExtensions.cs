...
using Flapjack.Search.Exceptions;
...
public partial interface ISearchClient
{
  ...
  /// <exception cref="FlapjackException"></exception>
  TimeSpan GetSecuredApiKeyRemainingValidity(string securedApiKey);

  /// <summary>
  /// Executes a synchronous search for the provided search requests, with certainty that we will only request Flapjack records (hits). Results will be received in the same order as the queries.
  /// </summary>
  ...
  /// <summary>
  /// Executes a synchronous search for the provided search requests, with certainty that we will only request Flapjack facets. Results will be received in the same order as the queries.
  /// </summary>
  /// <param name="requests">A list of search requests to be executed.</param>
  /// <param name="searchStrategy">The search strategy to be employed during the search. (optional)</param>
  /// <param name="options">Add extra http header or query parameters to Flapjack.</param>
  ...
  /// </summary>
  /// <param name="indexName">The index in which to perform the request.</param>
  /// <param name="objects">The list of `objects` to store in the given Flapjack `indexName`.</param>
  ...
  /// </summary>
  /// <param name="indexName">The index in which to perform the request.</param>
  /// <param name="objectIDs">The list of `objectIDs` to remove from the given Flapjack `indexName`.</param>
  /// <param name="waitForTasks">Whether or not we should wait until every `batch` tasks has been processed, this operation may slow the total execution time of this method but is more reliable..</param>
  /// <param name="batchSize">The size of the chunk of `objects`. The number of `batch` calls will be equal to `length(objects) / batchSize`. Defaults to 1000.</param>
  /// <param name="options">Add extra http header or query parameters to Flapjack.</param>
  /// <param name="cancellationToken">Cancellation Token to cancel the request.</param>
  ...
  /// </summary>
  /// <param name="indexName">The index in which to perform the request.</param>
  /// <param name="objects">The list of `objects` to update in the given Flapjack `indexName`.</param>
  ...
  /// in order to transform records before indexing them to Flapjack.
  ...
  /// <param name="options">Add extra http header or query parameters to Flapjack.</param>
  /// <param name="cancellationToken">Cancellation Token to cancel the request.</param>
  Task<List<Flapjack.Search.Models.Ingestion.WatchResponse>> SaveObjectsWithTransformationAsync(
    string indexName,
  ...
  List<Flapjack.Search.Models.Ingestion.WatchResponse> SaveObjectsWithTransformation(
    string indexName,
    IEnumerable<object> objects,
    bool waitForTasks = false,
    int batchSize = 1000,
  ...
  /// <param name="cancellationToken">Cancellation Token to cancel the request.</param>
  Task<
    List<Flapjack.Search.Models.Ingestion.WatchResponse>
  ...
  List<Flapjack.Search.Models.Ingestion.WatchResponse> PartialUpdateObjectsWithTransformation(
    string indexName,
    IEnumerable<object> objects,
    bool createIfNotExists = true,
    bool waitForTasks = false,
...
public partial class SearchClient : ISearchClient
{
  ...
  )
  {
    if (operation == ApiKeyOperation.Update)
    {
      if (apiKey == null)
      {
        throw new FlapjackException("`ApiKey` is required when waiting for an `update` operation.");
      }
  ...
  public TimeSpan GetSecuredApiKeyRemainingValidity(string securedApiKey)
  {
    ...
    if (matches.Count == 0)
    {
      throw new FlapjackException("validUntil not found in given secured api key.");
    }
  ...
  )
  {
    ...
    catch (FlapjackApiException ex) when (ex.HttpErrorCode == 404)
    {
  ...
  )
  {
    ...
    return await _ingestionTransporter
      .ChunkedPushAsync(
        indexName,
        objects,
        Flapjack.Search.Models.Ingestion.Action.AddObject,
        waitForTasks,
        batchSize,
  ...
  /// <inheritdoc/>
  public List<Flapjack.Search.Models.Ingestion.WatchResponse> SaveObjectsWithTransformation(
    string indexName,
    IEnumerable<object> objects,
    bool waitForTasks = false,
    int batchSize = 1000,
  ...
  )
  {
    ...
    var action = createIfNotExists
      ? Flapjack.Search.Models.Ingestion.Action.PartialUpdateObject
      : Flapjack.Search.Models.Ingestion.Action.PartialUpdateObjectNoCreate;
  ...
  /// <inheritdoc/>
  public List<Flapjack.Search.Models.Ingestion.WatchResponse> PartialUpdateObjectsWithTransformation(
    string indexName,
    IEnumerable<object> objects,
    bool createIfNotExists = true,
    bool waitForTasks = false,
