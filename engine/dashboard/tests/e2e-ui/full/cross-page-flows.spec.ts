...
test.describe('Cross-Page Flows', () => {
  ...
  test('create index, add documents, search, then delete', async ({ page, request }) => {
    ...
    // Step 2: Navigate to index search page and add a document via API
    await request.post(
      `${API_BASE}/1/indexes/${tempIndex}/documents`,
      {
        headers: API_HEADERS,
        data: [{ objectID: 'lifecycle-1', name: 'Lifecycle Test Product', brand: 'TestBrand' }],
      }
    );
    ...
    await expect(page.getByText('Lifecycle Test Product').first()).toBeVisible({ timeout: 10_000 });
  ...
  test('searching with analytics toggle populates analytics page', async ({ page }) => {
    ...
    // Look for the analytics toggle and enable it
    const analyticsToggle = page.getByLabel(/analytics/i).or(
      page.locator('button[role="switch"]')
    );
    if (await analyticsToggle.first().isVisible({ timeout: 3_000 }).catch(() => false)) {
    ...
    await searchInput.press('Enter');
    await expect(page.getByText('MacBook').first()).toBeVisible({ timeout: 10_000 });
    ...
    // Analytics page should show search data
    await expect(page.getByText('Search Analytics').first()).toBeVisible();
  });
