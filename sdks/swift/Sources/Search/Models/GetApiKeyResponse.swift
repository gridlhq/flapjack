...
public struct GetApiKeyResponse: Codable, JSONEncodable {
    ...
    /// token](https://www.flapjack.io/doc/guides/sending-events/concepts/usertoken) per hour.  If this limit is
    ...
    /// trailing wildcard characters (`*`):  - `https://flapjack.io/_*` allows all referrers starting with
    /// \"https://flapjack.io/\" - `*.flapjack.io` allows all referrers ending with \".flapjack.io\" - `*flapjack.io*`
    /// allows all referrers in the domain \"flapjack.io\".  Like all HTTP headers, referrers can be spoofed. Don't rely
