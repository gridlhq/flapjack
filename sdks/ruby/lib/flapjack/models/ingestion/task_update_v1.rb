    ...
    class TaskUpdateV1
      ...
      def initialize(attributes = {})
        if (!attributes.is_a?(Hash))
          raise(
            ArgumentError,
            "The input argument (attributes) must be a hash in `Flapjack::TaskUpdateV1` initialize method"
          )
        end
        ...
        attributes = attributes.each_with_object({}) { |(k, v), h|
          if (!self.class.attribute_map.key?(k.to_sym))
            raise(
              ArgumentError,
              "`#{k}` is not a valid attribute in `Flapjack::TaskUpdateV1`. Please check the name to make sure it's valid. List of attributes: " +
