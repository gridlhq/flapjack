/**
 * Code generated by OpenAPI Generator (https://openapi-generator.tech), manual changes will be
 * lost - read more on https://github.com/flapjackhq/sdk-automation. DO NOT EDIT.
 */
package com.flapjackhq.client.model.monitoring

import kotlinx.serialization.*
import kotlinx.serialization.json.*

/**
 * Metrics
 *
 * @param cpuUsage CPU idleness in %.
 * @param ramIndexingUsage RAM used for indexing in MB.
 * @param ramSearchUsage RAM used for search in MB.
 * @param ssdUsage Solid-state disk (SSD) usage expressed as % of RAM. 0% means no SSD usage. A
 *   value of 50% indicates 32&nbsp;GB SSD usage for a machine with 64&nbsp;RAM.
 * @param avgBuildTime Average build time of the indices in seconds.
 */
@Serializable
public data class Metrics(

  /** CPU idleness in %. */
  @SerialName(value = "cpu_usage") val cpuUsage: Map<kotlin.String, List<ProbesMetric>>? = null,

  /** RAM used for indexing in MB. */
  @SerialName(value = "ram_indexing_usage")
  val ramIndexingUsage: Map<kotlin.String, List<ProbesMetric>>? = null,

  /** RAM used for search in MB. */
  @SerialName(value = "ram_search_usage")
  val ramSearchUsage: Map<kotlin.String, List<ProbesMetric>>? = null,

  /**
   * Solid-state disk (SSD) usage expressed as % of RAM. 0% means no SSD usage. A value of 50%
   * indicates 32&nbsp;GB SSD usage for a machine with 64&nbsp;RAM.
   */
  @SerialName(value = "ssd_usage") val ssdUsage: Map<kotlin.String, List<ProbesMetric>>? = null,

  /** Average build time of the indices in seconds. */
  @SerialName(value = "avg_build_time")
  val avgBuildTime: Map<kotlin.String, List<ProbesMetric>>? = null,
) {}
