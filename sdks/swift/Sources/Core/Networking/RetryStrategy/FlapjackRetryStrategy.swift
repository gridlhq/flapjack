//
//  FlapjackRetryStrategy.swift
//
//
//  Created by Flapjack on 17/03/2020.
//
...
// MARK: - FlapjackRetryStrategy

// Algolia's retry strategy in case of server error, timeouts...

class FlapjackRetryStrategy: RetryStrategy {
    private var hosts: [RetryableHost]
    /// Concurrent synchronization queue
    private let queue = DispatchQueue(label: "FlapjackRetryStrategySync.queue")
    ...
    func isRetryable(_ error: Error) -> Bool {
        switch error {
        case let .requestError(error) as FlapjackError where error is URLError:
            true

        case let .httpError(httpError) as FlapjackError
    ...
    func isTimeout(_ error: Error) -> Bool {
        switch error {
        case let .requestError(error as URLError) as FlapjackError where error.code == .timedOut:
            true

        case let .httpError(error) as FlapjackError where error.statusCode == .requestTimeout:
            true
...
extension FlapjackRetryStrategy: CustomDebugStringConvertible {
