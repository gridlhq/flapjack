/**
 * Code generated by OpenAPI Generator (https://openapi-generator.tech), manual changes will be
 * lost - read more on https://github.com/flapjackhq/sdk-automation. DO NOT EDIT.
 */
package com.flapjackhq.client.model.search

import kotlinx.serialization.*
import kotlinx.serialization.json.*

/**
 * Request body for updating dictionary entries.
 *
 * @param requests List of additions and deletions to your dictionaries.
 * @param clearExistingDictionaryEntries Whether to replace all custom entries in the dictionary
 *   with the ones sent with this request.
 */
@Serializable
public data class BatchDictionaryEntriesParams(

  /** List of additions and deletions to your dictionaries. */
  @SerialName(value = "requests") val requests: List<BatchDictionaryEntriesRequest>,

  /**
   * Whether to replace all custom entries in the dictionary with the ones sent with this request.
   */
  @SerialName(value = "clearExistingDictionaryEntries")
  val clearExistingDictionaryEntries: Boolean? = null,
) {}
