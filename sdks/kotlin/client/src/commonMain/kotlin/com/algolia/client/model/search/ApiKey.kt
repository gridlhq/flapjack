 ...
 * API key object.
 *
 * @param acl Permissions that determine the type of API requests this key can make. The required
 *   ACL is listed in each endpoint's reference. For more information, see
 *   [access control list](https://www.flapjack.io/doc/guides/security/api-keys/#access-control-list-acl).
 ...
 *   [user token](https://www.flapjack.io/doc/guides/sending-events/concepts/usertoken) per hour. If
 *   this limit is reached, the API returns an error with status code `429`. By default, there's no
 *   limit.
 * @param queryParameters Query parameters to add when making API requests with this API key. To
 *   restrict this API key to specific IP addresses, add the `restrictSources` parameter. You can
 *   only add a single source, but you can provide a range of IP addresses. Creating an API key
 *   fails if the request is made from an IP address outside the restricted range.
 * @param referers Allowed HTTP referrers for this API key. By default, all referrers are allowed.
 *   You can use leading and trailing wildcard characters (`*`): - `https://flapjack.io/_*` allows
 *   all referrers starting with \"https://flapjack.io/\" - `*.flapjack.io` allows all referrers
 *   ending with \".flapjack.io\" - `*flapjack.io*` allows all referrers in the domain
 *   \"flapjack.io\". Like all HTTP headers, referrers can be spoofed. Don't rely on them to secure
 *   your data. For more information, see
 *   [HTTP referrer restrictions](https://www.flapjack.io/doc/guides/security/security-best-practices/#http-referrers-restrictions).
...
public data class ApiKey(
   ...
   * listed in each endpoint's reference. For more information, see
   * [access control list](https://www.flapjack.io/doc/guides/security/api-keys/#access-control-list-acl).
   */
   ...
   * leading and trailing wildcard characters (`*`): - `https://flapjack.io/_*` allows all referrers
   * starting with \"https://flapjack.io/\" - `*.flapjack.io` allows all referrers ending with
   * \".flapjack.io\" - `*flapjack.io*` allows all referrers in the domain \"flapjack.io\". Like all
