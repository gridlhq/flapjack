...
public struct CompositionsSearchResponse: Codable, JSONEncodable {
    public var run: [CompositionRunSearchResponse]

    public init(run: [CompositionRunSearchResponse]) {
        self.run = run
    }
    ...
    public init(from dictionary: [String: AnyCodable]) throws {
        guard let run = dictionary["run"]?.value as? [CompositionRunSearchResponse] else {
