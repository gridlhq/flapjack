...
public partial class ApiKey
{
  ...
  /// Initializes a new instance of the ApiKey class.
  /// </summary>
  /// <param name="acl">Permissions that determine the type of API requests this key can make. The required ACL is listed in each endpoint's reference. For more information, see [access control list](https://www.flapjack.io/doc/guides/security/api-keys/#access-control-list-acl).  (required).</param>
  ...
  /// <summary>
  /// Permissions that determine the type of API requests this key can make. The required ACL is listed in each endpoint's reference. For more information, see [access control list](https://www.flapjack.io/doc/guides/security/api-keys/#access-control-list-acl).
  /// </summary>
  /// <value>Permissions that determine the type of API requests this key can make. The required ACL is listed in each endpoint's reference. For more information, see [access control list](https://www.flapjack.io/doc/guides/security/api-keys/#access-control-list-acl). </value>
  ...
  /// <summary>
  /// Maximum number of API requests allowed per IP address or [user token](https://www.flapjack.io/doc/guides/sending-events/concepts/usertoken) per hour.  If this limit is reached, the API returns an error with status code `429`. By default, there's no limit.
  /// </summary>
  /// <value>Maximum number of API requests allowed per IP address or [user token](https://www.flapjack.io/doc/guides/sending-events/concepts/usertoken) per hour.  If this limit is reached, the API returns an error with status code `429`. By default, there's no limit. </value>
  ...
  /// <summary>
  /// Allowed HTTP referrers for this API key.  By default, all referrers are allowed. You can use leading and trailing wildcard characters (`*`):  - `https://flapjack.io/_*` allows all referrers starting with \"https://flapjack.io/\" - `*.flapjack.io` allows all referrers ending with \".flapjack.io\" - `*flapjack.io*` allows all referrers in the domain \"flapjack.io\".  Like all HTTP headers, referrers can be spoofed. Don't rely on them to secure your data. For more information, see [HTTP referrer restrictions](https://www.flapjack.io/doc/guides/security/security-best-practices/#http-referrers-restrictions).
  /// </summary>
  /// <value>Allowed HTTP referrers for this API key.  By default, all referrers are allowed. You can use leading and trailing wildcard characters (`*`):  - `https://flapjack.io/_*` allows all referrers starting with \"https://flapjack.io/\" - `*.flapjack.io` allows all referrers ending with \".flapjack.io\" - `*flapjack.io*` allows all referrers in the domain \"flapjack.io\".  Like all HTTP headers, referrers can be spoofed. Don't rely on them to secure your data. For more information, see [HTTP referrer restrictions](https://www.flapjack.io/doc/guides/security/security-best-practices/#http-referrers-restrictions). </value>
