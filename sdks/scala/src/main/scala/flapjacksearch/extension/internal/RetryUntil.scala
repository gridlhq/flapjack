package flapjacksearch.extension.internal

import flapjacksearch.exception.FlapjackWaitException

import scala.concurrent.duration._
import scala.concurrent.{ExecutionContext, Future, Promise}

private[flapjacksearch] object RetryUntil {
  ...
  def retryUntil[T](
    ...
    def attempt(retryCount: Int, currentDelay: Long): Future[T] = {
      if (retryCount >= maxRetries) {
        Future.failed(FlapjackWaitException("The maximum number of retries exceeded."))
      } else {
