...
module Flapjack
  module Transport
    ...
    class Transport
      ...
      # @param requester [Object] requester used for sending requests. Uses Flapjack::Http::HttpRequester by default
      #
      ...
      def request(call_type, method, path, body, opts = {})
        ...
        @retry_strategy.get_tryable_hosts(call_type).each do |host|
          ...
          if outcome == FAILURE
            ...
            if response.headers["content-type"]&.include?("text/html")
              raise Flapjack::FlapjackHttpError.new(response.status, response.reason_phrase)
            end

            decoded_error = JSON.parse(response.error, :symbolize_names => true)
            raise Flapjack::FlapjackHttpError.new(response.status, decoded_error[:message])
          end
        ...
        raise(
          Flapjack::FlapjackUnreachableHostError.new(
            "Unreachable hosts. If the error persists, check your Flapjack server status and network configuration.",
            retry_errors
          )
        )
      end
      ...
      def build_request(method, path, body, request_options)
        ...
        request[:query_params] = Flapjack::Transport.stringify_query_params(request_options.query_params)
