...
test.describe('Synonyms', () => {
  ...
  test('create a one-way synonym via dialog', async ({ page, request }) => {
    ...
    if (await typeSelect.isVisible({ timeout: 3_000 }).catch(() => false)) {
      await typeSelect.selectOption('oneWaySynonym');
    } else {
      // Try clicking type buttons/tabs
      const oneWayBtn = dialog.getByText('One-way').first();
      if (await oneWayBtn.isVisible().catch(() => false)) {
    ...
    const idInput = dialog.locator('input.font-mono').first();
    await idInput.clear();
    await idInput.fill('e2e-oneway-synonym');

    // Fill in the input word and synonyms
    const inputField = dialog.locator('input[placeholder*="input" i], input[placeholder*="Input" i]').first();
    if (await inputField.isVisible({ timeout: 3_000 }).catch(() => false)) {
      await inputField.fill('phone');
    }

    const synonymInputs = dialog.locator('input[placeholder^="Synonym"], input[placeholder^="Word"]');
    if (await synonymInputs.first().isVisible({ timeout: 3_000 }).catch(() => false)) {
      await synonymInputs.nth(0).fill('telephone');
      if (await synonymInputs.nth(1).isVisible().catch(() => false)) {
        await synonymInputs.nth(1).fill('cell');
      }
    }
