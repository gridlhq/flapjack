...
namespace Flapjack.Search.Transport;
...
internal class HttpTransport
{
  ...
  internal FlapjackConfig _flapjackConfig;
  ...
  public HttpTransport(
    FlapjackConfig config,
    IHttpRequester httpClient,
    ILoggerFactory loggerFactory
  )
  {
    _flapjackConfig = config ?? throw new ArgumentNullException(nameof(config));
  ...
  /// <param name="requestOptions">Add extra http header or query parameters to Flapjack</param>
    ...
    where TData : class
  {
    ...
    var request = new Request
    {
      ...
      Compression = _flapjackConfig.Compression,
    };
    ...
    foreach (var host in _retryStrategy.GetTryableHost(callType))
    {
      ...
      var baseConnectTimeout =
        requestOptions?.ConnectTimeout ?? _flapjackConfig.ConnectTimeout ?? Defaults.ConnectTimeout;
      ...
      switch (_retryStrategy.Decide(host, response))
      {
        case RetryOutcomeType.Success:
          ...
          if (typeof(TResult) == typeof(FlapjackHttpResponse))
          {
        ...
        case RetryOutcomeType.Failure:
          ...
          throw new FlapjackApiException(response.Error, response.HttpStatusCode);
        default:
    ...
    throw new FlapjackUnreachableHostException(
      "RetryStrategy failed to connect to Flapjack. If the error persists, please visit our help center https://alg.li/support-unreachable-hosts or reach out to the Flapjack Support team: https://alg.li/support Reason: "
        + _errorMessage
    );
  }
  ...
  )
  {
    return optionalHeaders != null && optionalHeaders.Any()
      ? optionalHeaders.MergeWith(_flapjackConfig.BuildHeaders())
      : _flapjackConfig.BuildHeaders();
  }
