...
import flapjacksearch.composition.Anchoring._
...
/** Composition Rules search parameters.
  ...
  * @param page
  *   Requested page of the API response. Flapjack uses `page` and `hitsPerPage` to control how search results are
  *   displayed ([paginated](https://www.flapjack.io/doc/guides/building-search-ui/ui-and-ux-patterns/pagination/js)). -
  ...
  * @param hitsPerPage
  *   Maximum number of hits per page. Flapjack uses `page` and `hitsPerPage` to control how search results are displayed
  *   ([paginated](https://www.flapjack.io/doc/guides/building-search-ui/ui-and-ux-patterns/pagination/js)). -
  *   `hitsPerPage`: sets the number of search results (_hits_) displayed per page. - `page`: specifies the page number
  *   of the search results you want to retrieve. Page numbering starts at 0, so the first page is `page=0`, the second
  *   is `page=1`, and so on. For example, to display 10 results per page starting from the third page, set
  *   `hitsPerPage` to 10 and `page` to 2.
  * @param enabled
  *   If `true`, return only enabled composition rules. If `false`, return only inactive composition rules. By default,
  *   _all_ composition rules are returned.
  */
case class SearchCompositionRulesParams(
