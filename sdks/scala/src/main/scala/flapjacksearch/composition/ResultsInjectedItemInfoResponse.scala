...
case class ResultsInjectedItemInfoResponse(
    key: String,
    appliedRules: Option[Seq[ResultsInjectedItemAppliedRulesInfoResponse]] = scala.None,
    additionalProperties: Option[List[JField]] = None
)

class ResultsInjectedItemInfoResponseSerializer extends Serializer[ResultsInjectedItemInfoResponse] {

  override def deserialize(implicit
      format: Formats
  ): PartialFunction[(TypeInfo, JValue), ResultsInjectedItemInfoResponse] = {
    case (TypeInfo(clazz, _), json) if clazz == classOf[ResultsInjectedItemInfoResponse] =>
      json match {
        case jobject: JObject =>
          val formats = format - this
          val mf = manifest[ResultsInjectedItemInfoResponse]
  ...
  override def serialize(implicit format: Formats): PartialFunction[Any, JValue] = {
    case value: ResultsInjectedItemInfoResponse =>
