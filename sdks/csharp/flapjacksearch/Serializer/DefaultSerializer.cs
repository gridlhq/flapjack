...
internal class DefaultJsonSerializer(ILoggerFactory logger) : ISerializer
{
  ...
  public string Serialize(object data)
  {
    ...
    catch (Exception ex)
    {
      ...
      throw new FlapjackException($"Error while serializing object of type {dataType}", ex);
    }
  }
  ...
  private async Task<object> Deserialize(Stream response, Type type)
  {
    ...
    catch (Exception ex)
    {
      ...
      throw new FlapjackException($"Error while deserializing response of type {type}", ex);
    }
  }
}
