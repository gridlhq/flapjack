...
package object util {
  ...
  private[flapjacksearch] def requireNotNull(param: Any, error: String): Unit = {
    if (param == null) throw new IllegalArgumentException(error)
  }

  /** Escape the given string to be used as URL query value.
    */
  private[flapjacksearch] def escape(input: Any): String = input match {
  ...
  /** Convert a parameter to a string.
    ...
    */
  private[flapjacksearch] def paramToString(value: Any): String = value match {
