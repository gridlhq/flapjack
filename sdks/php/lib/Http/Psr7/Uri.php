...
namespace Flapjack\FlapjackSearch\Http\Psr7;
 ...
 *
 * Adapted for PHP 5.3 by Flapjack.
 *
 * @internal
 */
class Uri implements UriInterface
{
    ...
    public static function withoutQueryValue(UriInterface $uri, $key)
    {
        ...
        $result = array_filter(explode('&', $current), function ($part) use (
            $decodedKey
        ) {
            $parts = explode('=', $part);
    ...
    public static function withQueryValue(UriInterface $uri, $key, $value)
    {
        ...
        if ('' === $current) {
            ...
            $decodedKey = rawurldecode($key);
            $result = array_filter(explode('&', $current), function (
                $part
            ) use ($decodedKey) {
                $parts = explode('=', $part);
    ...
    private function validateState()
    {
        if (
            '' === $this->host
            && ('http' === $this->scheme || 'https' === $this->scheme)
        ) {
            $this->host = self::HTTP_DEFAULT_HOST;
        }

        if ('' === $this->getAuthority()) {
            if (0 === mb_strpos($this->path, '//')) {
                throw new \InvalidArgumentException('The path of a URI without an authority must not start with two slashes "//"');
            }
            $parts = explode('/', $this->path, 2);
            if ('' === $this->scheme && false !== mb_strpos($parts[0], ':')) {
